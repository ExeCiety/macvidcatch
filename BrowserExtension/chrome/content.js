...
async function refreshYouTubeCandidateForClick() {
  ...
  lastYouTubeUrl = video.url;
  const config = await chrome.storage.local.get({ blocklist: [], allowlist: [], allowlistMode: false, showFloatingButton: true });
...
function showButtonIfEnabled() {
  chrome.storage.local.get({ showFloatingButton: true }, config => {
